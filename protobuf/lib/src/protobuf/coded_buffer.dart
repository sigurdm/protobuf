...
class _CodedBufferMerger {
  ...
  _CodedBufferMergingStatus mergeMore({int byteLimit = -1}) {
    ...
    while (byteLimit == -1 || input._bufferPos - start < byteLimit) {
      ...
      if (tag == 0) {
        stderr.writeln("$tag ${stack.map((e) => e._message.info_
            .messageName)} ${fieldInfoStack.map((fi) => fi.name)}");
        input.stopReadingMessage();
        _FieldSet subMessage = stack.removeLast();
        if (stack.isEmpty) {
          return _CodedBufferMergingStatus.done;
        }
        FieldInfo fieldInfo = fieldInfoStack.removeLast();
        if (fieldInfo.isRepeated) {
          stack.last._ensureRepeatedField(fieldInfo).add(subMessage._message);
        } else {
          stack.last._setFieldUnchecked(fieldInfo, subMessage._message);
        }
        continue;
      }
      ...
      if (fi == null || !_wireTypeMatches(fi.type, wireType)) {
        if (!currentFieldSet._ensureUnknownFields().mergeFieldFromBuffer(
            tag, input)) {
          // TODO: Why do we return here?
          return _CodedBufferMergingStatus.done;
        }
        continue;
      }
...
//        fs._setFieldUnchecked(fi, subMessage);
        ...
        case PbFieldType._OPTIONAL_MESSAGE:
          ...
          if (stack.length >= recursionLimit) {
            throw StateError("Recursion limit reached");
          }
          fieldInfoStack.add(fi);
          break;
...
//        fs._ensureRepeatedField(fi).add(subMessage);
        ...
        case PbFieldType._MAP:
          currentFieldSet._ensureMapField(fi).add(input, extensionRegistry);
          break;
        default:
